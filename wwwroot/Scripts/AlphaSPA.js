...
function Main(){
  ...
  const _1=Doc.Element("div", [], [Doc.Element("div", [Attr.Create("style", "\r\n                    position:fixed;\r\n                    top:0;\r\n                    left:0;\r\n                    width:100%;\r\n                    height:50px;\r\n                    background:#222;\r\n                    color:white;\r\n                    display:flex;\r\n                    align-items:center;\r\n                    padding:0 10px;\r\n                    z-index:3000;\r\n                ")], [Doc.Element("button", [Attr.Create("style", "font-size:20px; background:none; border:none; color:white; cursor:pointer;"), Attr.HandlerImpl("click", () =>() => menuOpen.Set(!menuOpen.Get()))], [Doc.TextView(Map((isOpen) => isOpen?"///":"|||", menuOpen.View))]), Doc.Element("div", [Attr.Create("style", "margin:10px")], [Doc.TextNode("Parking App")])]), Doc.Element("div", [Attr.Create("style", "display:flex; margin-top:50px; height:calc(100vh - 50px);")], [Doc.Element("div", [Dynamic("style", Map((isOpen) => isOpen?"width:200px; background:#333; color:white; padding:10px; transition:0.3s;":"width:0px; overflow:hidden; transition:0.3s;", menuOpen.View))], [Doc.Element("button", [Dynamic("style", Map((p) => p.$===0?"display:block; width:100%; padding:12px; margin:6px 0; background:#008CFF; color:white; border:none; border-radius:8px; text-align:left;":"display:block; width:100%; padding:12px; margin:6px 0; background:#444; color:white; border:none; border-radius:8px; text-align:left;", currentPage.View)), Attr.HandlerImpl("click", () =>() => currentPage.Set(Parking))], [Doc.TextNode("Parking")]), Doc.Element("button", [Dynamic("style", Map((p) => p.$===1?"display:block; width:100%; padding:12px; margin:6px 0; background:#008CFF; color:white; border:none; border-radius:8px; text-align:left;":"display:block; width:100%; padding:12px; margin:6px 0; background:#444; color:white; border:none; border-radius:8px; text-align:left;", currentPage.View)), Attr.HandlerImpl("click", () =>() => currentPage.Set(Payment))], [Doc.TextNode("Payment")])]), Doc.Element("div", [Attr.Create("style", "flex:1; padding:10px;")], [Doc.BindView((x) => x, mainView)])])]);
  LoadLocalTemplates("");
  Doc.RunById("main", _1);
}
class Object_1 {
